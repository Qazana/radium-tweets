...
spl_autoload_register( 'Radium_Tweets::autoload' );
...
class Radium_Tweets {
	...
	public function __construct() {
		...
		do_action_ref_array( 'radium_forms_init', array( $this ) );
	...
	public function activation() {
		...
		if ( version_compare( $wp_version, '3.0.0', '<' ) ) {
			...
			wp_die( printf( __( 'Sorry, but your version of WordPress, <strong>%s</strong>, does not meet the Radium Forms\'s required version of <strong>3.3.1</strong> to run properly. The plugin has been deactivated. <a href="%s">Click here to return to the Dashboard</a>', 'radium_forms' ), $wp_version, admin_url() ) );
		}
	...
	public function widget() {
        ...
        register_widget( 'Radium_Tweets_Widget' );

 	}

	/**
	 * Loads the plugin upgrader, registers features and
	...
	public function init() {
		...
		load_plugin_textdomain( 'radium_tweets', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );

        $integrate = new Radium_Tweets_Integrate();

        // Only run certain processes in the admin.
        if ( is_admin() ) :

             /** Instantiate all the necessary admin components of the plugin */
            $admin       = new Radium_Tweets_Admin();
            $adminassets = new Radium_Tweets_AdminAssets();
            $strings     = new Radium_Tweets_Strings();
